...
def parse_tweet(elem):
    try:
        ...
        ctime = datetime.strptime(published, "%Y-%m-%dT%H:%M:%S.000Z").replace(
            tzinfo=timezone.utc
        ).timestamp()
...
def collect_round(driver, existing, path):
    ...
    try:
        ...
        log.error("could not load timeline")
        return 0
...
def timeline(username, headless):
    ...
    with browser(headless) as driver:
        ...
        while True:
            ...
            total += n
            wait = BACKOFF + random.random() * BACKOFF_JITTER
    ...
    with browser(headless) as driver:
        ...
        for i, target in enumerate(targets):
            ...
            try:
                ...
                log.error(f"@{safe}: no timeline found")
                continue
