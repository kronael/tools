---
name: sonnet
description: "/sonnet — launch a high-effort background subagent for investigation, bug hunting, pre-review, exploration, and template-heavy coding. NOT for cheap mapping/mechanical work (use /haiku); escalate hard multi-file implementation to /opus."
...
- ALWAYS use `subagent_type: "sonnet"` on the Agent tool (NOT `model: "sonnet"`). The `sonnet` agent definition sets `model: sonnet` AND high effort — the only reliable way to set effort on a spawned agent.
- Use medium only when the task is clear enough that `/haiku` could plausibly do it, but `sonnet` is chosen for slightly better judgment or context handling.
