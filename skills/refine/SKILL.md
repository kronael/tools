...
6. **Document** - spawn `Task(agent="readme")`. Non-optional: refining a feature includes its docs, not a follow-up task for someone else. The readme agent MUST check `README.md` — and `ARCHITECTURE.md`/`CLAUDE.md` when the project keeps them and the change touches them — against what the code now does, and fix what drifted (new endpoints, flags, migrations, config, behavior), not just be spawned as a formality. List exactly what changed (file + one-line each, per the Prompt Structure below) so it has something concrete to reconcile against, not "go check for drift."
