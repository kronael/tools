---
name: create-code-presentation
description: Create a reveal.js presentation from a code talk document. Use when building a new feature/work presentation for a knowledge-sharing session. NOT for bug fixes or refactoring talks (different structure needed).
when_to_use: "create presentation, make slides, build reveal.js talk, knowledge session slides"
user-invocable: true
---

# Create Code Presentation (reveal.js)

For **new feature / new work** presentations. Bug fix or refactoring talks need a
different structure (leaner, focuses on root cause + diff, not problem/built/result).

## Repo structure

```
marinade-talks/YYYYMMDD_<slug>/
  ...
  talk/
    ...
    Makefile
    vite.config.js
    css/theme/source/marinade.scss
```

Scaffold by copying from an existing talk:
```bash
src=marinade-talks/20260205_using_claude_code/talk
dst=marinade-talks/YYYYMMDD_<slug>/talk
mkdir -p $dst/css/theme/source
cp $src/package.json $src/Makefile $src/vite.config.js $dst/
cp $src/css/theme/source/marinade.scss $dst/css/theme/source/
