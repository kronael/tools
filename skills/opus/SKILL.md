---
name: opus
description: /opus — launch a background subagent forcing model=opus. Use for complex reasoning, design decisions, and multi-file architecture work.
when_to_use: "do this in an opus sub", "spawn an opus sub", "use opus"
user-invocable: true
---

Launch the prompt after /opus as a background general-purpose agent (run_in_background: true, model: "opus").
...
- ALWAYS set the Agent tool's `model: "opus"`.
...
- For mechanical work or straightforward tasks, prefer `/sonnet` or `/haiku` — opus is for tasks requiring deep reasoning, complex architecture decisions, or synthesising across many files.
