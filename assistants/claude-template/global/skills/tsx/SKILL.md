...
- `"use client"` only on the smallest interactive leaf, NEVER at page level
...
- NEVER fetch in `useEffect` — server components or React Query/SWR
- Server Actions for form submission, `useActionState` for validation
- Forms SHOULD work without JS (progressive enhancement)

## Styling
- ALWAYS Tailwind theme variables, NEVER hardcoded colors or `style={{}}`
